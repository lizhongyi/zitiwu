...
*/
	...
	<form method="post" action="<?php echo U('Job/doCommand');?>">
	<div class="body_content">
    <div class="top_action"><a href="<?php echo U('Job/insert');?>">录入招聘</a> <a href="<?php echo U('Job/resume');?>" >应聘管理</a></div>
	<table cellspacing="0" cellpadding="0" id="maintable" class="formtable">
        ...
        <td><input type="checkbox" name="id[]" value="<?php echo ($vo["id"]); ?>"><?php echo ($vo["id"]); ?><input name="shuguangUpdateId[]" type="hidden" id="shuguangUpdateId[]" value="<?php echo ($vo["id"]); ?>" /></td>
        
		<td><input name="title[]" id="title[]" value="<?php echo ($vo["title"]); ?>" class="input-style" style="<?php echo ($vo["title_style"]); ?>">
		  <?php echo (statusicon($vo["status"],1,$frontUrl,'hidden.gif','隐藏')); echo (statusicon($vo["istop"],1,$frontUrl,'istop.gif','置顶')); ?><span style="text-align:left"><?php echo (attachstatus($vo["attach"],1,$frontUrl,'attach.png','附件')); ?> </span></td>
		<td><input name="number[]" id="number[]" value="<?php echo ($vo["number"]); ?>" size="5" /></td>
		<td>
		...
		<td><a href="<?php echo U('Job/resume',array('id'=>$vo['id']));?>"><img src="__PUBLIC__/Admin/search.gif" alt="查看应聘人员" align="absmiddle" /></a>　<a href="<?php echo U('Job/modify',array('id'=>$vo['id']));?>"><img src="__PUBLIC__/Admin/modify22px.gif" alt="编辑" align="absmiddle" /></a></td>
