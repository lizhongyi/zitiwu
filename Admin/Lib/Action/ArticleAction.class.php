...
class ArticleAction extends GlobalAction
{
    ...
    public function index()
    {
        ...
        $dataList = $this->dao->Table(C('DB_PREFIX').'article a')->Join(C('DB_PREFIX').'category b on a.category_id=b.id')->Field('a.*,b.title as category')->Order($setOrder)->Where($condition)->Limit($p->firstRow.','.$p->listRows)->select();
    ...
    {   global $rr;
        parent::_checkPermission('Article_insert');
    ...
    {
        parent::_checkPermission('Article_modify');
        parent::_setMethod('post');
    ...
    public function doCommand()
    {
        ...
        parent::_checkPermission('Article_command');
        if(getMethod() == 'get'){
