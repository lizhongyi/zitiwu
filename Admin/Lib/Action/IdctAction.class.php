 ...
 * @version        	$Id: IdcAction.class.php v1.0 2012-01-01 06:59:03 
...

{
...

    {
        ...
        $dataList = $this->dao->Table(C('DB_PREFIX').'Idc a')->Join(C('DB_PREFIX').'category b on a.category_id=b.id')->Field('a.*,b.title as category')->Order($setOrder)->Where($condition)->Limit($p->firstRow.','.$p->listRows)->select();
...

    {
        ...
        parent::_checkPermission('Idc_insert');
...

    {
        ...
        parent::_checkPermission('Idc_modify');
...

    {
        ...
        parent::_checkPermission('Idc_order');
        ...
        $dataList = $this->dao->Table(C('DB_PREFIX').'order a')->Join(C('DB_PREFIX').'Idc b on a.title_id=b.id')->Field('a.*,b.title')->Order($setOrder)->Where($condition)->Limit($p->firstRow.','.$p->listRows)->select();
...

    {
        ...
        $record = $this->dao->Table(C('DB_PREFIX').'order a')->Join(C('DB_PREFIX').'Idc b on a.title_id=b.id')->Field('a.*,b.title')->where('a.id='.$item)->find();

        if(empty($item) || empty($record)) parent::_message('error', '记录不存在', U('Idc/order'));
...

    {
        ...
        empty($item) && parent::_message('error','记录不存在', U('Idc/order'));;
...

        {
...

            {
                ...
                parent::_message('success', '更新完成', U('Idc/order'));

            }else

            {

                parent::_message('error', '更新失败', U('Idc/order'));
...

    {
        ...
        parent::_checkPermission('Idc_command');
        ...
        switch ($operate){
            ...
            case 'orderDelete': parent::_delete('Order', U('Idc/order'));break;
